...
class OneStepNnomialEnv:
    ...
    def step(self, delta, B):
        # Sample one terminal outcome (for info/trajectory only)
        ...
        # Replication metrics across all possible outcomes
        shortfalls = []
        for S_Ti, payoff_i in zip(self.possible_prices, self.payoffs):
            portfolio_i = delta * S_Ti + B
            shortfalls.append(max(0.0, payoff_i - portfolio_i))

        # Worst-case shortfall (super-replication constraint)
        max_shortfall = max(shortfalls)

        # Initial portfolio cost (same across states)
        cost = delta * self.S0 + B

        if max_shortfall > 1e-8:
            reward = -1e4   # still much worse than any valid hedge
        else:
            reward = -(cost / 100.0)
        # Next state
        ...
        info = {
            ...
            "portfolio": portfolio,
            "err": err,
            "shortfalls": shortfalls,     # keep full list for debugging if you want
...
if __name__ == "__main__":
    ...
    for _ in range(N):
        ...
        shortfalls_static.append(info["shortfalls"])
