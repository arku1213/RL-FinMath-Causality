...
if len(sys.argv) > 3:
    NUM_BINARIES = int(sys.argv[3])
if len(sys.argv) > 4:
    sigma = float(sys.argv[4])

print(f"\n{'='*60}")
print(f"CONFIGURATION:")
print(f"  N={N}, T_steps={T_steps}, NUM_BINARIES={NUM_BINARIES}, sigma={sigma}")
print(f"{'='*60}\n")
...
def calculate_n_nomial_equal_probabilities(N, sigma):
    ...
    # Create symmetric multipliers around 1.0
    if N % 2 == 1:  # Odd N
        middle_idx = N // 2
...
class NnomialPathSimulator:
    ...
    def simulate_single_path(self, _):
        """Simulate a single path (for parallel processing)."""
        path = []
        S = self.S0
    ...
    def simulate_paths(self, num_paths, parallel=True):
        """Simulate paths with optional parallelization."""
        if parallel and num_paths >= 1000:
            # Use multiprocessing for large batches
            num_workers = min(cpu_count(), 8)  # Limit to 8 cores
            with Pool(num_workers) as pool:
                paths = pool.map(self.simulate_single_path, range(num_paths))
            return paths
        else:
            # Serial execution for small batches
            return [self.simulate_single_path(None) for _ in range(num_paths)]
