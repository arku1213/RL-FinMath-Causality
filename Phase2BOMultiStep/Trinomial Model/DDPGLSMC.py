...
NUM_ITERATIONS = 8  # INCREASED from 6 for better convergence
BATCH_SIZE = 128
GAMMA = 0.99
TAU = 0.005
ACTOR_LR = 0.0003
...
SHORTFALL_MULTIPLIER = 500 + T_steps * 200  # Strong penalty for shortfall
COST_WEIGHT = 0.01  # Low - super-replication accepts higher cost
EXCESS_PENALTY = 0.01  # Low - excess is okay in super-replication

# Dynamic Action Scale - FIXED for super-replication
max_stock_price = S0 * (u ** T_steps)
max_terminal_payoff = max(max_stock_price - K, 0)

# FIXED: Adjust multiplier for super-replication (needs larger hedges)
if SUPER_REPLICATION:
    CONTINUATION_MULTIPLIER = 3.0 + (T_steps * 1.0) + 2.0  # Extra +2 for super-rep
else:
...
print("="*60)
print(f"LSMC + UNIVERSAL AGENT: TRINOMIAL T={T_steps}")
...
print(f"Cost Weight: {COST_WEIGHT} (LOW - accept higher cost)")
print(f"Excess Penalty: {EXCESS_PENALTY} (LOW - excess okay)")
print("="*60)
...
def calculate_trinomial_probabilities(S0, u, m, d, r, dt):
    ...
    assert abs(expected_growth - growth) < 1e-4, f"Expected growth mismatch"
    
    # Handle negative probabilities
    if p_u < 0 or p_m < 0 or p_d < 0:
        print("\nWARNING: Negative probabilities! Adjusting...")
        p_u = max(0.01, p_u)
        p_d = max(0.01, p_d)
        p_m = 1.0 - p_u - p_d
        print(f"Adjusted: p_u={p_u:.4f}, p_m={p_m:.4f}, p_d={p_d:.4f}")
...
# ============================================================
# PATH SIMULATION - TRINOMIAL
# ============================================================
class TrinomialPathSimulator:
    """Simulate paths through trinomial lattice"""
    ...
    def simulate_paths(self, num_paths):
        """Simulate random paths through trinomial lattice"""
        paths = []
...
def compute_reward(hedge, target_value, binary_prices, is_terminal):
    ...
    # FIXED: Extra penalty for high-value terminals (need stronger enforcement)
    if is_terminal and target_value > 30:
        penalty_weight = 200  # Double penalty for high-value states
    elif is_terminal:
        penalty_weight = 100
    else:
        penalty_weight = 150
...
def train_universal_agent_with_lsmc():
    ...
    print("MODE: SUPER-REPLICATION (TRINOMIAL)")
    print("="*60)
    ...
    state_dim = 3
    action_dim = 3  # 3 binaries for trinomial (will use 1 for terminals)
    ...
    for iteration in range(NUM_ITERATIONS):
        ...
        for episode in range(episodes_this_iter):
            ...
            if is_terminal:
                ...
                n_binaries = 3  # Trinomial has 3 binaries
                prices = [np.exp(-r * dt)] * 3
        ...
        if avg_shortfall < 0.5:  # Lowered threshold from 1.0
            print(f"\n🎉 SUCCESS! Avg Shortfall < 0.5 at iteration {iteration + 1}")
            break
