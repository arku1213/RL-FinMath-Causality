...
class TrinomialBinaryOptionEnvironment:
    """
    Trinomial model with ONLY binary options (NO BOND) for replication
    ...
    Complete market: 3 scenarios, 3 instruments (b1, b2, b3)
    ...
    def __init__(self, S0, K, r, T, factors):
        ...
        print(f"  Unique analytical solution: b1 = {self.b1_target:.4f}, b2 = {self.b2_target:.4f}, b3 = {self.b3_target:.4f}")
    
    def get_context(self):
        """Get context for bandit (static in this case)"""
        return np.array([self.S0, self.K], dtype=np.float32)
    
    def evaluate_replication(self, b1, b2, b3):
        """
        Evaluate how well a given (b1, b2, b3) replicates the option
        ...
        cost = b1 * self.binary_prices[0] + b2 * self.binary_prices[1] + b3 * self.binary_prices[2]
...
class ThompsonSamplingBandit:
    ...
    def select_action(self, context, exploration_factor=1.0):
        ...
        if len(self.observations) < 100:  # Initial exploration
            ...
            b3s = [obs[2] for obs in self.observations]
...
def train_thompson_sampling(env, n_rounds=20000, print_every=2000):
    ...
    print("Instruments: Binary_1 (b1), Binary_2 (b2), Binary_3 (b3)")
    print("Goal: Portfolio value = Option payoff in ALL scenarios")
    print("Unique solution: b1 = C_1, b2 = C_2, b3 = C_3")
    ...
    for round_num in range(n_rounds):
        ...
        b1, b2, b3 = bandit.select_action(context, exploration_factor)
        
        # Evaluate
        reward, cost, errors, portfolio_values = env.evaluate_replication(b1, b2, b3)
        
        # Track best solution (lowest replication error)
        total_error = np.sum(np.abs(errors))
        if total_error < best_error:
            best_error = total_error
            best_solution = (b1, b2, b3, cost, errors, portfolio_values)
        
        # Update bandit
        bandit.update(context, b1, b2, b3, reward)
        ...
        if (round_num + 1) % print_every == 0:
            ...
            if best_solution:
                b1_best, b2_best, b3_best, cost_best, errs_best, vals_best = best_solution
                print(f"  Best solution so far:")
                print(f"    b1={b1_best:.4f}, b2={b2_best:.4f}, b3={b3_best:.4f}")
...
def evaluate_final_solution(bandit, env, n_samples=5000):
    ...
    for _ in range(n_samples):
        b1, b2, b3 = bandit.select_action(context, exploration_factor=0.05)
        reward, cost, errors, portfolio_values = env.evaluate_replication(b1, b2, b3)
        
        total_error = np.sum(np.abs(errors))
        all_solutions.append((b1, b2, b3, cost, errors, portfolio_values, total_error))
    ...
    if best_solution:
        b1, b2, b3, cost, errors, portfolio_values = best_solution
        ...
        errors_all = [sol[6] for sol in all_solutions]
...
def main_trinomial():
    ...
    print("Instruments: Binary_1 + Binary_2 + Binary_3 (NO BOND)")
    print("Market: EXACTLY COMPLETE (3 scenarios, 3 instruments)")
    print("UNIQUE SOLUTION: b1 = C_1, b2 = C_2, b3 = C_3")
    ...
    b1_ana, b2_ana, b3_ana, cost_ana = find_analytical_solution(env)
    ...
    if best_solution:
        b1_ts, b2_ts, b3_ts, cost_ts, _, _ = best_solution
        
        print("\n" + "="*60)
        print("THOMPSON SAMPLING vs ANALYTICAL")
        print("="*60)
        print(f"Thompson Sampling: b1={b1_ts:.6f}, b2={b2_ts:.6f}, b3={b3_ts:.6f}, Cost={cost_ts:.6f}")
        print(f"Analytical:        b1={b1_ana:.6f}, b2={b2_ana:.6f}, b3={b3_ana:.6f}, Cost={cost_ana:.6f}")
        print(f"\nErrors:")
